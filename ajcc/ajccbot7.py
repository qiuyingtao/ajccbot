...
from_clock = cf.getint('interval', 'from')
latest_time = cf.getint('latest', 'hour')

from_clock_str = str(from_clock)
if len(from_clock_str) == 1:
    from_clock_str = '0' + from_clock_str
...
if (from_clock_str == currentHM[0:2] and re.search(r'^0[0-4]$', currentHM[2:4])) or re.search(r'^[0-4]$', str((int(currentHM) - from_clock * 100) % (interval_time * 100))): #控制间隔播报时间，每天从from_clock开始第一次播报，由于可能前面程序运行时间超过1分钟，所以除了要判断00分外，还要判断01、02、03、04分
