...
def heartbeat(latest_time, access_token, post_url, tw_api):
    ...
    msgWeibo = u'#中纪委纪律审查# 播报机器人竭诚为您服务，中纪委官网纪律审查页面在最近%s小时之内没有发布新的信息。更新时间：%s' % (latest_time, currentDateTime)
    msgTweet = u'#中纪委纪律审查 播报机器人竭诚为您服务，中纪委官网纪律审查页面在最近%s小时之内没有发布新的信息。更新时间：%s' % (latest_time, currentDateTime)
...
url = 'http://www.ccdi.gov.cn/jlsc/index'
...
try:
    ...
    #print htmldata
    ulStartIndex = htmldata.find('<ul class="list_news_dl fixed">')
    htmldata = htmldata[ulStartIndex:]
    ulEndIndex = htmldata.find('</ul>')
    ulStr = htmldata[:ulEndIndex+5]
    #print ulStr
    while hasAvailableLi:
        ...
        linkRelativeUrl = liStr[linkUrlStartIndex+10:linkUrlEndIndex-2]
        if linkRelativeUrl == 'ttp://www.ccdi.gov.cn/jlsc':
            ulStr = ulStr[liEndIndex+5:] 
            continue
        postDate = int(linkRelativeUrl[9:17])
        liStr2 = liStr[linkUrlEndIndex:]
        linkTxtStartIndex = liStr2.find('</i>')
        linkTxtEndIndex = liStr2.find('</a>')
        linkUrl = str('http://www.ccdi.gov.cn/jlsc' + linkRelativeUrl)
        linkTxt = liStr2[linkTxtStartIndex+4:linkTxtEndIndex].strip()
        if stickIndex < stick_count: # 对置顶的信息处理时不比较发布日期
            urlandtxtList.append((linkUrl, linkTxt.strip()))
    ...
    if len(newUrlandtxtList) != 0:
        for (urlItem,txtItem) in (newUrlandtxtList):
            try:
                ...
                msgWeibo = u'%s，来源：%s，更新时间：%s #中纪委纪律审查# 温馨提示：万一您发现链接打不开，说明该网页已被编辑撤下。' % (txtItem, urlItem, timeStr)
                msgTweet = u'%s，来源：%s，更新时间：%s #中纪委纪律审查 温馨提示：万一您发现链接打不开，说明该网页已被编辑撤下。' % (txtItem, urlItem, timeStr)
                broadcast(access_token, post_url, tw_api, msgWeibo, msgTweet)
...
rebroadcast_oclock_list = cf.get('rebroadcast', 'oclock').split(',')
latest_time = cf.getint('latest', 'hour')

for rebroadcast_oclock in rebroadcast_oclock_list:
    if (rebroadcast_oclock == currentHM[0:2] and re.search(r'^0[0-4]$', currentHM[2:4])): #控制滚动重播时间，在config.ini里设定重播时间，这里读出来，和当前时间小时数匹配上就行，由于可能前面程序运行时间超过1分钟，所以除了要判断00分外，还要判断01、02、03、04分
            ...
            if len(newUrlandtxtInXHourList) == 0:
                ...
                for newUrlandtxtInXHourItem in newUrlandtxtInXHourList:
                    try:
                        ...
                        msgWeibo = u'【%s小时滚动播报】%s，来源：%s #中纪委纪律审查# 温馨提示：万一您发现链接打不开，说明该网页已被编辑撤下。' % (latest_time, ymdhUrlTxtList[2].decode('utf-8'), ymdhUrlTxtList[1].decode('utf-8'))
                        msgTweet = u'【%s小时滚动播报】%s，来源：%s #中纪委纪律审查 温馨提示：万一您发现链接打不开，说明该网页已被编辑撤下。%s' % (latest_time, ymdhUrlTxtList[2].decode('utf-8'), ymdhUrlTxtList[1].decode('utf-8'), index)
                        broadcast(access_token, post_url, tw_api, msgWeibo, msgTweet)
